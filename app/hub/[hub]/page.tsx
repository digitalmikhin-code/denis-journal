...
function ProjectsGuide(): JSX.Element {
  return (
    <section aria-labelledby="projects-guide-title" className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-soft dark:border-slate-800 dark:bg-slate-900 md:p-10">
      <h2 id="projects-guide-title" className="text-3xl font-black tracking-tight text-slate-950 dark:text-slate-50">
        Проектное управление: от цели до принятого результата
      </h2>
      <div className="mt-6 max-w-3xl space-y-5 leading-8 text-slate-700 dark:text-slate-300">
        <p>
          Проектное управление связывает ожидаемый результат с решениями о составе работы, сроках,
          ресурсах и ответственности. План помогает согласовать эти решения, а наблюдение за
          выполнением — вовремя пересматривать предположения. Количество закрытых задач само по себе
          не показывает, получил ли заказчик нужный результат.
        </p>
        <p>
          В материалах Дениса Михина проект рассматривается как система взаимосвязанных решений.
          Изменение приоритета влияет на загрузку, загрузка — на движение работы, а задержки — на
          ожидания участников. Поэтому полезно обсуждать не только статус задач, но и условия,
          которые помогают или мешают их завершать.
        </p>
        <h3 className="text-xl font-bold text-slate-950 dark:text-slate-50">О чём договориться до начала работы</h3>
        <p>
          <strong>Цель</strong> объясняет, ради какого изменения запускается проект.
          <strong> Результат</strong> описывает, что должно быть создано или изменено.
          <strong> Критерии приёмки</strong> позволяют проверить, соответствует ли выполненная работа
          договорённостям. Приёмка результата и оценка его последующего эффекта могут происходить
          в разное время; для каждой нужны понятный ответственный и способ проверки.
        </p>
        <p>
          <strong>Границы проекта</strong> определяют, какая работа входит в договорённости.
          <strong> Зависимость</strong> связывает продвижение одной работы с другой задачей,
          решением или внешним участником. <strong>Риск</strong> описывает возможное событие,
          которое повлияет на проект; уже возникшее препятствие требует конкретного решения.
          Важно заранее определить, кто может менять объём работы и согласовывать последствия.
        </p>
        <h3 className="text-xl font-bold text-slate-950 dark:text-slate-50">Как поддерживать управляемость</h3>
        <p>
          Работу стоит разбивать на проверяемые промежуточные результаты. Для ближайшего этапа
          уточняют владельца, доступные ресурсы и зависимости. Затем команда регулярно сопоставляет
          план с фактическим продвижением: где возникло ожидание, какое решение требуется и остаётся
          ли достижимым согласованный результат. Частота таких обсуждений зависит от темпа изменений.
        </p>
        <p>
          Учебный пример: запуск внутреннего сервиса задерживается на согласовании прав доступа.
          Дополнительные задачи разработчикам не устранят это ожидание. Сначала нужно уточнить,
          кто принимает решение, каких данных ему не хватает и когда они будут готовы. Если для
          ускорения предлагают сократить объём запуска, заказчик должен понимать, как это изменит
          результат и критерии его приёмки.
        </p>
        <p>
          Когда одна команда участвует в нескольких проектах, сроки зависят от общих приоритетов
          и доступности людей. Локальный план не разрешает конфликт ресурсов. Его нужно вынести
          на уровень, где можно выбрать, какую работу продолжать, отложить или остановить, и
          пересогласовать обязательства с участниками.
        </p>
        <h3 className="text-xl font-bold text-slate-950 dark:text-slate-50">Выберите материал под свою ситуацию</h3>
        <ul className="list-disc space-y-4 pl-5">
          <li>
            Для общего подхода прочитайте{" "}
            <Link href="/article/kak-upravlyat-proektami-spokoyno-i-sistemno/" className="font-semibold text-brand underline underline-offset-4">как управлять проектами спокойно и системно</Link> —
            о ясности результата, приоритетах, ответственности и прозрачности.
          </li>
          <li>
            Если работа постоянно переключается на срочные запросы, начните с материала{" "}
            <Link href="/article/kak-vystroit-ritm-raboty-proekta/" className="font-semibold text-brand underline underline-offset-4">о ритме работы проекта</Link>:
            планировании, синхронизации и точках управленческих решений.
          </li>
          <li>
            Если решение уже принято, но исполнение застревает, откройте разбор{" "}
            <Link href="/article/kak-sobirat-sistemu-realizatsii/" className="font-semibold text-brand underline underline-offset-4">системы реализации</Link> —
            полномочий, ограничений загрузки и передачи результата.
          </li>
        </ul>
        <h3 className="text-xl font-bold text-slate-950 dark:text-slate-50">Завершение проекта и следующий шаг</h3>
        <p>
          Завершение предполагает проверку результата и передачу его тем, кто будет им пользоваться
          и поддерживать. Если эффект проявится позднее, нужно договориться, кто и когда его оценит.
          Универсальный набор встреч не гарантирует успеха: порядок работы следует соотносить
          с неопределённостью, требованиями к качеству и зависимостями конкретного проекта.
        </p>
        <p>
          Для последовательного изучения есть программа{" "}
          <Link href="/training/259560/" className="font-semibold text-brand underline underline-offset-4">«Основы управления проектами»</Link>.
          Для разбора проектной работы компании можно изучить{" "}
          <Link href="/consulting/" className="font-semibold text-brand underline underline-offset-4">направления консалтинга</Link>.
          Дополнительные статьи собраны ниже на странице.
        </p>
      </div>
    </section>
  );
