...
> = {
  youtube: {
    ...
    accent: "text-brand",
    button: "border-slate-300 bg-white text-slate-950",
    poster: "from-slate-950 via-slate-900 to-brand",
    posterGlow: "bg-white/10",
    posterAccent: "bg-brand"
  },
  rutube: {
    chip: "bg-slate-100 text-slate-600 border-slate-200",
    surface: "from-white via-white to-white",
    ...
    <>
      <div className="grid gap-6 xl:grid-cols-2">
        {videos.map((item, index) => {
          ...
          return (
            <article
              key={item.slug}
              className={`overflow-hidden border border-slate-200 bg-gradient-to-br ${theme.surface} shadow-[0_18px_44px_rgba(9,22,43,0.06)]`}
            >
              <div className={`relative aspect-video overflow-hidden bg-gradient-to-br ${theme.poster}`}>
                ...
                >
                  <div className="absolute left-6 top-6 flex items-center gap-3">
                    <span className="border border-white/35 bg-white/12 px-3 py-1 text-[0.72rem] font-bold uppercase tracking-[0.14em] text-white backdrop-blur">
                  ...
                  <div className={`absolute bottom-0 right-0 h-24 w-40 ${theme.posterGlow} [clip-path:polygon(42%_0,100%_0,100%_100%,0_100%)]`} />

                  <div className="absolute inset-0 flex items-center justify-center">
                    <span className="flex h-16 w-16 items-center justify-center border border-white/70 bg-white/90 text-2xl text-slate-900 shadow-[0_16px_30px_rgba(15,23,42,0.22)] transition group-hover:scale-105">
                      ▶
                    </span>
                  </div>

                  <div className="absolute bottom-6 right-6">
                    <span className={`px-4 py-2 text-sm font-semibold text-white ${theme.posterAccent}`}>
                      Смотреть выпуск
                    </span>
                  </div>
                </button>
              ...
              <div className="flex flex-col justify-between gap-6 p-6">
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`inline-flex border px-3 py-1 text-[0.72rem] font-bold uppercase tracking-[0.14em] ${theme.chip}`}>
                      {PLATFORM_LABEL[item.frontmatter.platform] || item.frontmatter.platform}
                    </span>
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <button
                    type="button"
                    onClick={() => setActiveSlug(item.slug)}
                    className={`inline-flex border px-5 py-3 text-sm font-semibold transition hover:border-brand hover:text-brand ${theme.button}`}
                  >
                    Смотреть на сайте
                  </button>
                  <Link
      ...
      {activeVideo ? (
        <div className="fixed inset-0 z-[120] flex items-center justify-center p-4 sm:p-6">
          ...
          />
          <div className="relative z-[121] w-full max-w-6xl overflow-hidden border border-white/15 bg-slate-950 shadow-[0_30px_90px_rgba(15,23,42,0.48)]">
            <div className="flex items-center justify-between gap-4 border-b border-white/10 px-5 py-4 text-white sm:px-6">
              ...
              <button
                type="button"
                onClick={() => setActiveSlug(null)}
                className="border border-white/20 px-4 py-2 text-sm font-semibold text-white/90 transition hover:bg-white/10"
              >
                Закрыть
              </button>
            </div>
