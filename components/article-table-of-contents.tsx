      ...
      <ol className="mt-4 space-y-2">
        {items.map((item, index) => (
          <li key={item.id} className={item.level === 3 ? "pl-4" : ""}>
            <a
              href={`#${item.id}`}
              className="group flex gap-3 rounded-2xl px-3 py-2.5 text-sm font-semibold leading-5 text-slate-700 transition hover:bg-slate-50 hover:text-slate-950 dark:text-slate-200 dark:hover:bg-slate-800 dark:hover:text-white"
            >
              <span className="mt-0.5 text-[11px] font-black text-slate-400 group-hover:text-slate-500">
