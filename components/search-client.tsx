  ...
  return (
    <section className="space-y-6">
      <form
        ...
        className="border border-slate-200 bg-white p-5 shadow-[0_18px_44px_rgba(9,22,43,0.06)] dark:border-slate-800 dark:bg-slate-900 md:p-7"
      >
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-[1fr_auto]">
          <input
            ...
            className="w-full border border-slate-200 bg-slate-50 px-5 py-4 text-base font-bold text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-brand focus:bg-white focus:ring-4 focus:ring-brand/10 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
          />
          <button
            type="submit"
            className="bg-slate-950 px-6 py-4 text-sm font-black text-white transition hover:bg-brand dark:bg-white dark:text-slate-950"
          >
            Найти
          </button>
        </div>
  ...
  return (
    <label className="block">
      ...
      <select
        ...
        className="mt-2 w-full border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-bold text-slate-700 outline-none dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
      >
...
function NextStepCard({ item, related }: { item: SearchResult; related: SearchResult[] }): JSX.Element {
  return (
    <section className="border border-slate-200 bg-white p-6 shadow-[0_18px_44px_rgba(9,22,43,0.06)] dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="mt-4 grid gap-5 lg:grid-cols-[1fr_auto] lg:items-center">
        ...
        <Link
          href={item.href}
          className="bg-slate-950 px-5 py-3 text-center text-sm font-black text-white transition hover:bg-brand dark:bg-white dark:text-slate-950"
        >
          {item.actionLabel}
        </Link>
      </div>
        ...
        <div className="mt-5 flex flex-wrap gap-2">
          {related.map((relatedItem) => (
            <Link
              ...
              className="border border-slate-200 bg-white px-3 py-1.5 text-xs font-bold text-slate-700 transition hover:border-brand hover:text-brand dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200"
            >
...
function ResultCard({ item }: { item: SearchResult }): JSX.Element {
  return (
    <article className="border border-slate-200 bg-white p-5 shadow-[0_16px_34px_rgba(9,22,43,0.05)] transition hover:border-brand dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-wrap items-center gap-2">
        <span className="border border-slate-200 bg-slate-100 px-2.5 py-1 text-[11px] font-black uppercase tracking-[0.12em] text-slate-500 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-300">
          {item.label}
        </span>
      ...
      <div className="mt-3 grid gap-2">
        {items.map((item) => (
          <Link
            key={item.id}
            href={item.href}
            className="border border-slate-200 p-3 text-sm font-bold transition hover:border-brand hover:text-brand dark:border-slate-700"
          >
            {item.title}
          </Link>
        ))}
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {queries.map((item) => (
          <button
            key={item}
            type="button"
            className="border border-slate-200 px-3 py-1.5 text-xs font-bold text-slate-700 transition hover:border-brand hover:text-brand dark:border-slate-700 dark:text-slate-200"
